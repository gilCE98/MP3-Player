...
//   uint16_t result = 1;
//   LPC_ADC->CR |= (1 << 5); // set channel
//   result = ((LPC_ADC->DR[5] >> 4)) & 0x0FFF;
 ...
 */
uint16_t adc__get_channel_reading_with_burst_mode(uint8_t channel_number) { return (LPC_ADC->DR[channel_number] >> 4); }